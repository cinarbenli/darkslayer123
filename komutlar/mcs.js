  ...
  if (message.content.startsWith("mcsunucu")) {
      var url = 'http://mcapi.us/server/status?ip=' + args[0] + '&port=' + mcPort;
        ...
        request(url, function (err, response, body) {
            if (err) {
                console.log(err);
                return message.channel.sendEmbed(new Discord.RichEmbed().setTitle('Hata!').addField('Sunucu bilgileri alınırken beklenmedik bir hatayla karşılaştık.').setThumbnail("https://cdn.pixabay.com/photo/2013/07/12/19/25/minecraft-154749_960_720.png").setAuthor("").setFooter('').setColor("RANDOM").setTimestamp());
            }
...
exports.conf = {
  enabled: true,
  guildOnly: false,
  aliases: ['mcsunucu', 'mc sunucu','mcs'],
  permLevel: 0
};
exports.help = {
  name: 'mcsunucu',
